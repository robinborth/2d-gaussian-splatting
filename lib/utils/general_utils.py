...
def colormap(img, cmap=None, dpi=300):
    W, H = img.shape[:2]  # Extract height and width
    ...
    # Convert to PyTorch tensor (Normalize and change channel order)
    img_tensor = (
        torch.from_numpy(data[:, :, :3] / 255.0).float().permute(2, 0, 1)
    )  # Convert to RGB
