...
def compute_full_loss_map(image, gt_image, lambda_dssim: float):
    l1_loss = torch.abs(image - gt_image)
    ssim_loss = ssim_map(image, gt_image)
    loss_map = (1.0 - lambda_dssim) * l1_loss + lambda_dssim * (1.0 - ssim_loss)
    return loss_map
