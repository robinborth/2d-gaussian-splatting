...
class Viewer:
    def __init__(
        ...
        self.cameras_json = (
            os.path.join(self.model_paths, "cameras.json")
    ...
    def _init_models(self, iterations):
        ...
        self.viewer_renderer = ViewerRenderer(
            self.gaussian_model, self.background_color, not self.is_training
        )
    ...
    def _get_training_gaussians(self, new_gaussians):
        ...
        self.gaussian_model._features_rest = (
            new_gaussians._features_rest.clone().detach()
        )
        self.viewer_renderer = ViewerRenderer(
            self.gaussian_model, self.background_color, self.is_training
        )
        ...
        if self.show_render_panel:
            with tabs.add_tab("Render"):
                self.render_panel = RenderPanel(
                    ...
                    background_color=self.background_color.detach()
                    .cpu()
                    .numpy()
                    .tolist(),
        ...
        with tabs.add_tab("General"):
            if self.is_training:
                with server.add_gui_folder("Training Infos"):
                    ...
                    self.fps = server.add_gui_text("fps", initial_value=" frame/sec")
            else:
            ...
            with server.add_gui_folder("Gaussian Model"):
                ...
                self.surfel_mode = server.add_gui_button_group(
                    "View Type", ("ptc", "disk")
                )
